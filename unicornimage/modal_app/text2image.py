...
def encode_prompt_chunked(
    ...
    pos_input_ids_1_raw = (
        tokenizer(prompt, add_special_tokens=False, truncation=False, return_tensors="pt").input_ids[0].tolist()
    )
    pos_input_ids_2_raw = (
        tokenizer_2(prompt, add_special_tokens=False, truncation=False, return_tensors="pt").input_ids[0].tolist()
    )
    neg_input_ids_1_raw = (
        tokenizer(negative_prompt or "", add_special_tokens=False, truncation=False, return_tensors="pt")
        .input_ids[0]
        .tolist()
    )  # Handle empty string
    neg_input_ids_2_raw = (
        tokenizer_2(negative_prompt or "", add_special_tokens=False, truncation=False, return_tensors="pt")
        .input_ids[0]
        .tolist()
    )  # Handle empty string
    ...
    num_special_tokens = (
        tokenizer.num_special_tokens_to_add(pair=False) if hasattr(tokenizer, "num_special_tokens_to_add") else 2
    )
    effective_chunk_size = max_length - num_special_tokens

    if effective_chunk_size <= 0:
        raise ValueError(
            f"Effective chunk size is too small or zero: {effective_chunk_size}. Check MAX_TOKEN_LENGTH and tokenizer special tokens."
        )
    ...
    with torch.no_grad():
        for i in range(num_chunks):
            ...
            if hasattr(pos_chunk_output_2, "text_embeds"):
                ...
                logging.getLogger("text2image").warning(
                    "No pooled embeddings found, falling back to first token of last hidden state"
                )
            ...
            if pos_chunk_embeds_1.shape[1] != seq_len:
                pos_chunk_embeds_1 = torch.nn.functional.pad(
                    pos_chunk_embeds_1, (0, 0, 0, seq_len - pos_chunk_embeds_1.shape[1]), value=0
                )
            if pos_chunk_embeds_2.shape[1] != seq_len:
                pos_chunk_embeds_2 = torch.nn.functional.pad(
                    pos_chunk_embeds_2, (0, 0, 0, seq_len - pos_chunk_embeds_2.shape[1]), value=0
                )
            ...
            if hasattr(neg_chunk_output_2, "text_embeds"):
                ...
                # Fallback - use the last hidden state's first token ([CLS]/[BOS]) as pooled representation
                logging.getLogger("text2image").warning(
                    "No pooled embeddings found for negative prompt, falling back to first token of last hidden state"
                )
            ...
            if neg_chunk_embeds_1.shape[1] != seq_len_neg:
                neg_chunk_embeds_1 = torch.nn.functional.pad(
                    neg_chunk_embeds_1, (0, 0, 0, seq_len_neg - neg_chunk_embeds_1.shape[1]), value=0
                )
            if neg_chunk_embeds_2.shape[1] != seq_len_neg:
                neg_chunk_embeds_2 = torch.nn.functional.pad(
                    neg_chunk_embeds_2, (0, 0, 0, seq_len_neg - neg_chunk_embeds_2.shape[1]), value=0
                )
            ...
            if final_negative_prompt_embeds is None:  # First chunk
                ...
                final_negative_prompt_embeds = torch.cat(
                    [final_negative_prompt_embeds, chunk_combined_neg_embeds], dim=1
                )
        ...
        if torch.isnan(prompt_embeds).any() or torch.isinf(prompt_embeds).any():
            logging.getLogger("text2image").warning(
                "NaN or Inf values detected in final positive embeddings. Replacing with zeros."
            )
        ...
        if torch.isnan(negative_prompt_embeds).any() or torch.isinf(negative_prompt_embeds).any():
            logging.getLogger("text2image").warning(
                "NaN or Inf values detected in final negative embeddings. Replacing with zeros."
            )
        ...
        if torch.isnan(pooled_prompt_embeds).any() or torch.isinf(pooled_prompt_embeds).any():
            logging.getLogger("text2image").warning(
                "NaN or Inf values detected in final pooled positive embeddings. Replacing with zeros."
            )
        ...
        if torch.isnan(negative_pooled_prompt_embeds).any() or torch.isinf(negative_pooled_prompt_embeds).any():
            logging.getLogger("text2image").warning(
                "NaN or Inf values detected in final pooled negative embeddings. Replacing with zeros."
            )
    ...
    logging.getLogger("text2image").debug(
        f"Negative embeds shape: {negative_prompt_embeds.shape}, negative pooled shape: {negative_pooled_prompt_embeds.shape}"
    )
...
SchedulerType = Literal["euler_ancestral", "dpmpp_2m_karras"]
...
def parse_loras(loras_json: str | None) -> list[dict[str, Any]] | None:
    if not loras_json:
        return None
    try:
        ...
        logger.error(f"Error parsing LoRAs: {e!s}")
        return None
...
class Inference:
    ...
    AVAILABLE_SCHEDULERS = {"euler_ancestral": "Euler Ancestral", "dpmpp_2m_karras": "DPM++ 2M Karras"}
    ...
    def setup(self):
        ...
        if self.load_default_model:
            ...
            try:
                ...
                logger.warning(f"Failed to pre-load default model: {e!s}")
    ...
    def _download_hf_lora(self, lora_id):
        ...
        if "/" in repo_id_path and not repo_id_path.endswith("/"):
            ...
            try:
                ...
                raise ValueError(f"Error downloading LoRA from Hugging Face: {e!s}")
            ...
            if lru_model_key != model_key and lru_model_key in self.loaded_models:
                ...
                try:
                    ...
                    if torch.cuda.is_available():
                        ...
                        logger.info("Cleared CUDA cache, freeing GPU memory")
            ...
            try:
                ...
                if model_id.startswith("civitai:") or model_id != DEFAULT_MODEL_ID:
                    try:
                        ...
                        vae = AutoencoderKL.from_pretrained("madebyollin/sdxl-vae-fp16-fix", torch_dtype=target_dtype)
                        pipeline.vae = vae.to("cuda")
                        logger.info("Successfully loaded and set 'madebyollin/sdxl-vae-fp16-fix' VAE.")
                    except Exception as e:
                        logger.warning(
                            f"Could not load 'madebyollin/sdxl-vae-fp16-fix': {e}. Using model's default VAE."
                        )
                ...
                try:
                    ...
                    logger.warning(f"Could not enable optimized attention: {e!s}")
                ...
                if loras:
                    ...
                    for lora_index, lora_spec in enumerate(loras):
                        ...
                        try:
                            ...
                            failed_loras.append(f"LoRA {lora_model_id}: Download failed - {e!s}")
                            continue

                        logger.info(
                            f"Applying LoRA {lora_index + 1}/{len(loras)} from {lora_path} with weight {weight}"
                        )
                        try:
                            ...
                            logger.error(f"Error applying LoRA {lora_model_id}: {e!s}")
                            failed_loras.append(f"LoRA {lora_model_id}: Application failed - {e!s}")
                    ...
                    if successful_loras:
                        logger.info(
                            f"Successfully loaded {len(successful_loras)}/{len(loras)} LoRAs: {', '.join(successful_loras)}"
                        )
                    if failed_loras:
                ...
                logger.error(f"Failed to load SDXL model: {e!s}")
                raise ValueError(f"Could not load SDXL model {model_id}: {e!s}")
        ...
        if clip_skip and clip_skip > 1:
            ...
            if hasattr(pipe, "text_encoder") and num_layers_to_actually_skip > 0:
                ...
                if num_layers_to_actually_skip < original_layers1:
                    logger.info(
                        f"Applying CLIP skip {clip_skip} to text_encoder (original layers: {original_layers1}, using {original_layers1 - num_layers_to_actually_skip})"
                    )
                    pipe.text_encoder.config.num_hidden_layers = original_layers1 - num_layers_to_actually_skip
                else:
                    logger.warning(
                        f"clip_skip={clip_skip} is too high for text_encoder with {original_layers1} layers. Not applying skip."
                    )
            ...
            if hasattr(pipe, "text_encoder_2") and num_layers_to_actually_skip > 0:
                ...
                if num_layers_to_actually_skip < original_layers2:
                    logger.info(
                        f"Applying CLIP skip {clip_skip} to text_encoder_2 (original layers: {original_layers2}, using {original_layers2 - num_layers_to_actually_skip})"
                    )
                    pipe.text_encoder_2.config.num_hidden_layers = original_layers2 - num_layers_to_actually_skip
                else:
                    logger.warning(
                        f"clip_skip={clip_skip} is too high for text_encoder_2 with {original_layers2} layers. Not applying skip."
                    )
