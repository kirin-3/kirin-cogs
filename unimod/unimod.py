...
class AIAnalysisResult:
    ...
    explanation: str
    primary_message_id: Optional[int]
...
Analyze this conversation against the server rules, paying close attention to channel-specific rules. Respond with JSON only."""
    ...
    def _build_alert_embed(self, guild: discord.Guild, channel: discord.TextChannel, messages: List[BufferedMessage], result: AIAnalysisResult) -> discord.Embed:
        ...
        embed = discord.Embed(
            title="⚠️ Potential Rule Violation Detected",
            color=color,
    ...
    async def on_message(self, message: discord.Message):
        ...
        if should_process:
            asyncio.create_task(
                self._process_buffer(message.guild, message.channel, messages_snapshot, threshold)
            )
