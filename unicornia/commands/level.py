...
class LevelCommands:
    ...
    async def _level_check_logic(self, ctx, member: discord.Member = None):
        ...
        try:
            ...
            embed = discord.Embed(title=f"{member.display_name}'s Level", color=member.color or discord.Color.blue())
            ...
            embed.add_field(
                name="Progress",
                value=f"`{progress_bar}` {level_stats.level_xp / level_stats.required_xp:.1%}",
                inline=False,
            )
        ...
        **Examples**
        ...
        try:
            ...
            if success:
                ...
                await ctx.send("<a:zz_NoTick:729318761655435355> Failed to award XP.")
