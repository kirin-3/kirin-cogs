...
class BackgroundShopView(discord.ui.View):
    ...
    async def get_embed(self):
        ...
        embed = discord.Embed(
            title=f"🖼️ XP Background: {name}", description=f"**Price:** {price_text}\n{desc}", color=discord.Color.blue()
        )
        if url:
    ...
    async def buy_button(self, interaction: discord.Interaction, button: discord.ui.Button):
        ...
        if key in self.user_owned:
            ...
            if success:
                ...
                await interaction.response.send_message(
                    "<a:zz_NoTick:729318761655435355> Failed to equip.", ephemeral=True
                )
            return
        ...
        if price == -1:
            await interaction.response.send_message(
                "<a:zz_NoTick:729318761655435355> This item is unavailable.", ephemeral=True
            )
            return
        ...
        if user_balance < price:
            currency_symbol = await self.ctx.cog.config.currency_symbol()
            await interaction.response.send_message(
                f"<a:zz_NoTick:729318761655435355> Insufficient funds. You need {price:,} {currency_symbol}.",
                ephemeral=True,
            )
            return
        ...
        **Examples**
        ...
        try:
            ...
            if success:
                ...
                embed = discord.Embed(
                    title="✅ Purchase Successful!", description=description, color=discord.Color.green()
                )
        ...
        **Examples**
        ...
        try:
            ...
            if entry_type == self.db.shop.SHOP_TYPE_ROLE:
                ...
                if not details:
                    await ctx.send(
                        '<a:zz_NoTick:729318761655435355> Please specify a role for role items. Usage: `[p]shop add role 1000 "VIP Role" @VIPRole`'
                    )
                    return
            ...
            item_id = await self.shop_system.add_shop_item(
                ctx.guild.id, next_index, price, name, ctx.author.id, entry_type, role_name, role_id, None, None
            )
        ...
        **Examples**
        ...
        try:
            ...
            if field == "price":
                ...
                await ctx.send(
                    "<a:zz_NoTick:729318761655435355> Invalid field. Available fields: name, price, type, role, req"
                )
                return
    ...
    async def shop_backgrounds(self, ctx):
        ...
        **Syntax**
        ...
        try:
            ...
            await ctx.reply(
                "<a:zz_NoTick:729318761655435355> Error loading backgrounds. Please check the configuration file.",
            ...
            await ctx.reply(
                "<a:zz_NoTick:729318761655435355> An unexpected error occurred while loading backgrounds.",
                mention_author=False,
            )
    ...
    async def shop_owned(self, ctx):
        ...
        **Syntax**
        ...
        try:
            ...
            if not owned_items:
                await ctx.reply(
                    "<a:zz_NoTick:729318761655435355> You don't own any backgrounds yet. Use `[p]xpshop backgrounds` to see what's available!",
                    mention_author=False,
                )
                return
            ...
            if not owned_backgrounds:
                await ctx.reply(
                    "<a:zz_NoTick:729318761655435355> You own backgrounds, but none of them are currently available in the shop configuration.",
                    mention_author=False,
                )
                return
            ...
            await ctx.reply(
                f"<a:zz_NoTick:729318761655435355> Error loading owned backgrounds: {e}", mention_author=False
            )
