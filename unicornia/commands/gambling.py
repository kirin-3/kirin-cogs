...
class GamblingCommands(UnicorniaMixinBase):
        ...
        try:
            ...
            if not success:
                if result.get("error") == "insufficient_funds":
                    ...
                    await ctx.reply(
                        f"<a:zz_NoTick:729318761655435355> You don't have enough {currency_symbol} Slut points. You have {currency_symbol}{result['balance']:,}.",
                        mention_author=False,
                    )
                else:
                    await ctx.reply(
                        f"<a:zz_NoTick:729318761655435355> Error: {result.get('error', 'Unknown error')}",
                        mention_author=False,
                    )
                return
    ...
    async def gambling_rps(self, ctx, choice: str | None = None, amount: str = "0"):
        """
        ...
        try:
            ...
            if not success:
                if result.get("error") == "insufficient_funds":
                    ...
                    await ctx.reply(
                        "<a:zz_NoTick:729318761655435355> Please choose: rock, paper, or scissors", mention_author=False
                    )
                else:
                    await ctx.reply(
                        f"<a:zz_NoTick:729318761655435355> Error: {result.get('error', 'Unknown error')}",
            ...
            if amount_int > 0:
                if result["result"] == "win":
                    ...
                    await ctx.reply(
                        f"{result['user_choice']} vs {result['bot_choice']} - You lost {currency_symbol}{result['loss_amount']:,}!",
                        mention_author=False,
                    )
                else:
                    await ctx.reply(
                        f"{result['user_choice']} vs {result['bot_choice']} - It's a draw! No Slut points lost.",
                        mention_author=False,
                    )
            else:
                if result["result"] == "win":
                    await ctx.reply(
                        f"{result['user_choice']} vs {result['bot_choice']} - You win!", mention_author=False
                    )
                elif result["result"] == "lose":
                    await ctx.reply(
                        f"{result['user_choice']} vs {result['bot_choice']} - You lose!", mention_author=False
                    )
                else:
                    await ctx.reply(
                        f"{result['user_choice']} vs {result['bot_choice']} - It's a draw!", mention_author=False
                    )
        ...
        **Examples**
        ...
        try:
            ...
            if result["won_amount"] > 0:
                await ctx.reply(
                    f"🎰 **{rolls_str}** - {result['win_type'].replace('_', ' ').title()}! You won {currency_symbol}{result['won_amount']:,}!",
                    mention_author=False,
                )
            else:
                await ctx.reply(
    ...
    async def gambling_betflip(self, ctx, amount: str, guess: str | None = None):
        """
        Bet on a coin flip.
        ...
        try:
            ...
            if not success:
                if result.get("error") == "insufficient_funds":
                    ...
                    await ctx.reply(
                        "<a:zz_NoTick:729318761655435355> Invalid guess. Please choose 'heads' or 'tails'.",
                        mention_author=False,
                    )
                else:
                    await ctx.reply(
            ...
            if result["won"]:
                ...
                embed.add_field(
                    name="Result",
                    value=f"You guessed {result['guess']} and won {currency_symbol}{result['win_amount']:,}!",
                    inline=False,
                )
            else:
                ...
                embed.add_field(
                    name="Result",
                    value=f"You guessed {result['guess']} and lost {currency_symbol}{result['loss_amount']:,}.",
                    inline=False,
                )
    ...
    async def gambling_mines(self, ctx, amount: str | None = None, mines: int = 3):
        """
