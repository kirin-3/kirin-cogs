...
class StockQuickBuyModal(ui.Modal):
    ...
    def __init__(self, market_system):
        ...
        self.symbol_input = ui.TextInput(
            label="Stock Symbol", placeholder="e.g. UNICORN", min_length=1, max_length=10, style=discord.TextStyle.short
        )
        self.amount_input = ui.TextInput(
            label="Amount (Shares)",
            placeholder="Enter number of shares",
...
class StockPortfolioView(ui.LayoutView):
    ...
    def update_components(self):
        ...
        # Header
        header_text = "## 📈 Portfolio Overview\n"
...
class StockListView(ui.LayoutView):
    ...
    def update_components(self):
        ...
        container.add_item(
            ui.TextDisplay(content=f"## 📋 All Stocks (Live Prices) - Page {self.current_page + 1}/{total_pages}")
        )
        container.add_item(ui.Separator())
...
class StockDashboardView(ui.LayoutView):
    ...
    def update_components(self):
        ...
        if self.market_system.top_expensive:
            expensive_text = generate_list_text(self.market_system.top_expensive, lambda s: "")
        ...
        if self.market_system.top_changed:
            changed_text = generate_list_text(self.market_system.top_changed, lambda s: "")
        ...
        if self.market_system.top_held:
            held_text = generate_list_text(
                self.market_system.top_held, lambda s: f"| 👥 Circ: {s.get('held_shares', 0):,}"
            )
        ...
        quick_buy_btn = ui.Button(
            label="Quick Buy", style=discord.ButtonStyle.success, custom_id="market:quick_buy", row=0
        )
        browse_btn = ui.Button(
            label="Browse Market", style=discord.ButtonStyle.primary, custom_id="market:browse", row=0
        )
        ...
        portfolio_btn = ui.Button(
            label="My Portfolio", style=discord.ButtonStyle.secondary, custom_id="market:portfolio", row=1
        )
        show_all_btn = ui.Button(
            label="Show All Stocks", style=discord.ButtonStyle.secondary, custom_id="market:show_all", row=1
        )
    ...
    async def browse_button(self, interaction: discord.Interaction):
        ...
        await interaction.response.send_message(
            "Browse stocks to buy:", view=StockBuySelectView(self.market_system, stocks), ephemeral=True
        )
    ...
    async def sell_button(self, interaction: discord.Interaction):
        ...
        # Send Ephemeral View for Selection
        await interaction.response.send_message(
            "Select a stock to sell:", view=StockSellSelectView(self.market_system, holdings), ephemeral=True
        )
