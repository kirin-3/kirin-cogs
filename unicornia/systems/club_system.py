...
class ClubSystem:
    ...
    async def get_members(self, club_id: int) -> list[ClubMember]:
        ...
        return [
            {"user_id": m[0], "username": m[1], "avatar_id": m[2], "total_xp": m[3], "is_admin": bool(m[4])}
            for m in members
        ]
    ...
    async def get_applicants(self, user: discord.Member) -> tuple[list[ClubUserInfo] | None, str]:
