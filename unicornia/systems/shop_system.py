...
class ShopSystem:
    ...
    async def get_shop_item(self, guild_id: int, item_id: int) -> ShopItem | None:
        ...
        if role_to_add:
            try:
                ...
                await self.db.economy.add_currency(
                    ...
                    "shop_refund",
                    str(item["id"]),
                    note=f"Refund for failed role assignment: {item['name']}",
                )
    ...
    async def add_shop_item(
        ...
        return await self.db.shop.add_shop_entry(
            guild_id, index, price, name, author_id, item_type, role_name, role_id, role_requirement, command
        )
    ...
    async def get_user_inventory(self, guild_id: int, user_id: int) -> list[UserInventoryItem]:
        ...
        for row in rows:
            ...
            inventory.append(
                {"id": row[0], "quantity": row[1], "index": row[2], "name": row[3], "price": row[4], "type": row[5]}
            )
