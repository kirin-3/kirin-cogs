...
class CurrencyGeneration:
    ...
    async def process_message(self, message: discord.Message):
        ...
        if is_fake:
            msg_content = (
                f"A fake {amount}{self.currency_symbol} has appeared! Pick them up by typing `&pick` or `.pick`"
            )
        else:
            msg_content = (
                f"A wild {amount}{self.currency_symbol} has appeared! Pick them up by typing `&pick` or `.pick`"
            )
    ...
    async def pick_plant(self, user_id: int, channel_id: int) -> tuple[int, list[int], bool] | None:
        ...
        async with self.db._get_connection() as db:
            ...
            for uid, amt in wallet_users:
                if uid not in user_balances:
                    user_balances[uid] = {"wallet": 0, "bank": 0}
            ...
            for user_id, balances in user_balances.items():
                ...
                if wallet_decay > 0:
                    ...
                    transactions.append(
                        (user_id, -wallet_decay, "decay", "system", f"Wallet decay: {decay_percent:.1%}")
                    )
