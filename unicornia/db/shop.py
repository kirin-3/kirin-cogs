class ShopRepository:
    ...
    async def get_shop_entry(self, guild_id: int, entry_id: int) -> tuple | None:
    ...
    async def _get_shop_entry(self, guild_id: int, entry_id: int, db) -> tuple | None:
    ...
    async def get_shop_entry_by_index(self, guild_id: int, index: int) -> tuple | None:
    ...
    async def purchase_shop_item(self, user_id: int, guild_id: int, entry_id: int) -> tuple[bool, str]:
        ...
        async with self.db._get_connection() as db:
            ...
            try:
                ...
                if price > 0:
                    success = await self.db.economy._remove_currency(
                        user_id, price, "shop_purchase", str(entry_id), other_id=None, note=f"Purchased: {name}", db=db
                    )

                    if not success:
                        await db.execute("ROLLBACK")
                        return False, "Insufficient currency or transaction failed."
