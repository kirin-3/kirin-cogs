...
class RateResponder(BaseTextResponder):
    ...
    async def respond(self, message: discord.Message, target: discord.Member, match: re.Match):
        ...
        # topic is not found
        responder = self.rate_classes.get(topic.lower(), self.rate_classes["default"])

        responder.topic = topic

        self.parent.logger.debug(f"Calling respond method from {responder.__class__.__name__}")
