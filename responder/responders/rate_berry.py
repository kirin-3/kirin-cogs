...
class BerryRate(BaseRateResponder):
    ...
    def get_berry_property(self, property: str, target: discord.Member, berry_name: str) -> Any:
        ...
        if target.id in self.user_overrides:
            value = self.user_overrides[target.id].get(property, getattr(self, property))
            return value
        else:
            value = self.berry_types[berry_name].get(property)
    ...
    async def respond(
        ...
        title = self.get_berry_property("title", target, berry_name)
