...
class TextDraw(ImageUtil):
    ...
    @staticmethod
    def get_contrast_color(color: str, dark_color: str = "black", light_color: str = "white") -> str:
    ...
    def draw_text(
        ...
        if outline:
            ...
            self.draw_text_outline(draw, text_centered, text, font, outline_color, outline_width)
        draw.text(text_centered, text, font=font, fill=color)

    @staticmethod
    def rotate_text_image(text_image: Image.Image, text_rotation: int = 0) -> Image.Image:
        """Rotates the text image by the given angle."""
        if text_rotation != 0:
            return text_image.rotate(text_rotation, expand=True, resample=Image.Resampling.BICUBIC)
        return text_image

    @staticmethod
    def calculate_offset(text_position: Position, rotated_size: tuple[int, int]) -> Position:
